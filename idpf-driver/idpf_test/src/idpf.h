...
#define IDPF_DEBUGFS_VC_OPC		122 /* Virtchnl opcode for debugfs */
#define VIRTCHNL_OP_MBX_PROXY		5020
...
struct idpf_debugfs_response {
	void *data;		/* data buffer */
	size_t data_len;	/* length of data buffer */
	struct mutex mutex;	/* lock to protect response data */
};
...
struct idpf_adapter {
	...
	struct idpf_debugfs_response debugfs_res;
#endif /* CONFIG_DEBUG_FS */
...
#else
static inline void idpf_debugfs_register(struct idpf_adapter *adapter) {}
static inline void idpf_debugfs_unregister(struct idpf_adapter *adapter) {}
static inline int idpf_debugfs_init(void) {}
static inline void idpf_debugfs_exit(void) {}
static inline void idpf_debugfs_mb_receive(struct idpf_adapter *adapter,
					   struct idpf_ctlq_msg *msg) {}
#endif /* CONFIG_DEBUG_FS */
