			       ...
			       struct idpf_tx_buf *first)
{
	...
	if (!params->offload.desc_ts[0] &&
	    !params->offload.desc_ts[1] &&
	    !params->offload.desc_ts[2])
					...
					struct idpf_queue *tx_q)
{
	...
	if (test_bit(__IDPF_Q_FLOW_SCH_EN, tx_q->flags)) {
		bool dft_match = idpf_dft_check(tx_q->vport, skb, &tx_params);

		if (unlikely(test_bit(__IDPF_Q_ETF_EN, tx_q->flags) || dft_match))
